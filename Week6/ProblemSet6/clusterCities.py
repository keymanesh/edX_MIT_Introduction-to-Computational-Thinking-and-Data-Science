...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        closest_distance = self.points[0].distance(other.points[0])    
    ...
    def maxLinkageDist(self, other):
        ...
        Euclidean dist between 2 points, defined in Point."""
        # TO DO
        fartherst_distance = self.points[0].distance(other.points[0])    
        for point1 in self.points:
            for point2 in other.points:
                dist = point1.distance(point2)
    ...
    def averageLinkageDist(self, other):
        ...
        for point1 in self.points:
            for point2 in other.points:
                avg_distance += point1.distance(point2)
                
        avg_distance = avg_distance / (len(self.points) * len(other.points))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        merged_cluster = Cluster(list(set(c1.points + c2.points)), c1.pointType)
    ...
    def findClosest(self, linkage):
        ...
        similarity = linkage(self.members[0], self.members[1])
        similar_c = (self.members[0], self.members[1])
        for c1_ind in range(len(self.members)):
            for c2_ind in range(c1_ind+1, len(self.members)):
        
                tmp =  linkage(self.members[c1_ind], self.members[c2_ind])
                if tmp < similarity:
                    similarity = tmp
                    similar_c = (self.members[c1_ind], self.members[c2_ind])
...
def test():
    print "points = buildCityPoints('cityTemps.txt', False)"
    points = buildCityPoints('cityTemps.txt', False)
    hCluster(points, Cluster.singleLinkageDist, 5, False)
    print "points = buildCityPoints('cityTemps.txt', True)"
